...
// Loader function to fetch menu items based on the category
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  // Redirect to the starter menu if the category is invalid or not provided
  if (!params.category || !categories.includes(params.category))
    return redirect("/menu/starter");

  // Convert the category to uppercase to match the database enum
  let category = params.category.toUpperCase() as Category;

  // Define search criteria for the database query
  let searchItems = {};

  // If the category is not "all", filter by the specific category
  ...
  // Fetch menu items from the database, ordered by ID
  let data = await prisma.menuItem.findMany({
    ...searchItems,
    orderBy: { id: "asc" },
  });

  // Return the fetched data and the category
      ...
      {filteredItems.length > 0 && (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredItems.map((menuItem) => (
            <Link
              to={`/menu/${category.toLowerCase()}/${menuItem.id.toString()}`}
              key={menuItem.id}
